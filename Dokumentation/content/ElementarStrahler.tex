...
Mit wachsendem Abstand zur Strahlungsquelle können einige Terme vernachlässigt werden. Brüche mit dem  Abstand R in höherer Potenz im Nenner werden vereinfacht zu Null. Für das Fernfeld eines Hertzschen Dipols ergeben sich daher die folgenden Beschreibungen \cite{elliott1981antenna}:
...
\end{figure}
\newpage 
Im Zentrum bildet sich ein magnetisches, zeitabhängiges Moment \textit{m(t)}. Stellt der Hertzsche Dipol eine E-Feld Antenne dar, so ist der Fitzgeraldsche Dipol eine H-Feld Antenne. In der unmittelbaren Nähe der Leiterschleife bildet sich ein sehr starkes Magnetfeld aus. Das Nahfeld des Fitzgeraldschen Dipols wird mit den folgenden drei Formeln (\ref{Fitz_Nah_Hr}, \ref{Fitz_Nah_Etheta} und \ref{Fitz_Nah_Ephi}) beschrieben\cite{elliott1981antenna}:
...
\label{Fitz_Nah_Ephi}
\end{equation}
Das zeitabhängige magnetische Moment \textit{m(t)} ergibt sich durch die Multiplikation der durch die Windung aufgespannten Fläche $a^{2}\pi=S$ mit dem  Schleifenstrom $I$. Der Strom $I$ ist auf der ganzen Windung konstant. Dank der Annahme des konstanten Stroms kann wie in (\ref{eq:magnetischesMoment}) gezeigt, das magnetische Moment $m(t)$ berechnet werden \cite{Harrington-TimeHarmonic}: 
\begin{equation}\label{eq:magnetischesMoment}
Ia^{2}\pi=IS=m(t)
\end{equation}
Wie beim Hertzschen Dipol können die Terme aus (\ref{Fitz_Nah_Hr}, \ref{Fitz_Nah_Etheta} und \ref{Fitz_Nah_Ephi}) mit R in der zweiten oder dritten Potenz im Nenner vernachlässigt werden für das Fernfeld. Die Amplituden des E- und H-Feldes sinken mit wachsendem Abstand $R$. Das Fernfeld kann daher wie folgt beschrieben werden:
