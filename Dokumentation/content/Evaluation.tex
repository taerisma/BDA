...
Das Abstrahlverhalten ist in den Kapiteln \ref{sec:DipolAntenne} und \ref{sec:LoopAntenneTheorie} aufgezeigt. Die Abbildung \ref{fig:FerdVektorEinheistsvektor} zeigt einen Poynting-Vektor im  kugelkoordinaten und im kartesischen Koordinatensystem. Am Ende des Poynting-Vektor $\vec{r}$  sind die Einheitsvektoren mit den dazugehörenden  E und H Feldvektoren ersichtlich\cite{Emant}. Der Vektor $\vec{E}_r$ zeigt die Richtung des Leistungstransport.\\
...
Wobei $\varepsilon_r$ die effektive Permittivitätszahl
 und $\mu_r$ die Permeabilitätszahl des Mediums ist. Beide Grössen sind im Allgemeinen frequenzabhängig. Für die Berechnung des Verkürzungsfaktors in Leitungen werden kurze Rechteckpulse betrachtet, die hohen Frequenzen beinhalten, bei denen sich $\varepsilon_r$ einem Grenzwert nähert. Für Leitungen aus Kupfer und Aluminium gilt weiterhin, dass $\mu_r \approx 1$ genspricht. Bei einer verlustfreien Leitung gilt\cite{Verkuertzungsfaktor_wiki}
...
Die Nahfeldeigenschaften der Dipol Antenne zeigen ein starkes E Feld. Dieses zeigt eine Richtungsabhängigkeit  des Winkels $\theta$. Die starken E Felder polarisieren alle Dielektrika in unmittelbarer Nähe des Dipols. Dies führt zu dielektrischen Verlusten. Diese steigen mit der Frequenz des Wechselfeldes. Die dielektrischen Verluste können als Widerstände in Serie zu den idealen Komponenten eines Ersatzschaltbildes gedacht werden. Sie stellen die äquivalente Verlustleistung an einer Komponente dar. Die Verlustenergie wird für das polarisieren der umgebenden Dielektrikum Struktur benötigt. Die Ausrichtungsenergie ist Verlustenergie und kann somit nicht abgestrahlt werden.
...
Das Abstrahlverhalten einer Dipolantenne ist $\theta$ abhängig. Der Richtfaktor oder auf englisch \textit{directivity} D genannt, ist bei Dipolantennen wie in Formel \ref{eq:Directivity} gezeigt und entspricht  1.64dBi.  Der Richtfaktor vergleicht die Leistungsdichte in der stärksten Abstrahlrichtung, mit der Leistungsdichte eines isotropen Kugelstrahlers  bei gleicher totaler abgestrahlter Leistung. Da die Dipolantenne in der Längsausrichtung des \glqq Connect 1\grqq \ Geräts untergebracht wird, ergibt sich eine theoretische Nullstelle des elektromagnetischen Feldes in der Verlängerung der Antennenstruktur. Die beiden Abbildungen \ref{fig:xyFeld} und \ref{fig:xzFeld} versuchen diese Nullstelen schematisch zu zeigen. Beide Abbildungen zeigen je einen Schnitt durch das elektromagnetische Feld, welches von dem Dipol abgestrahlt wird. Die durch dicke schwarze Striche dargestellten Felder rotationssymmetrisch sind zeigen die beiden Abbildungen \ref{fig:xyFeld} und \ref{fig:xzFeld}. Der Feldschwund, der  das \glqq Connect 1\grqq \ Gerät verursacht, ist in den beiden Abbildungen nicht gezeigt.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[!ht]%ESB Antenne
	\begin{center}
	\begin{tikzpicture}[scale = 0.8]
	\draw[line width=1.5pt](5, 5) rectangle (8, 10) node at (6.5, 9.5) {Connect 1};
	\draw[line width=1.5pt](5.3, 6.2) rectangle (7.7, 9);
	\draw[line width=1.5pt, ->, >=latex](1, 1) -- (3, ) node at (3.2, 1) {x};
	\draw[line width=1.5pt, ->, >=latex](1, 1) -- (1, 3) node at (1.2, 3) {y};
	\draw[line width=1.5pt](1, 1) circle (0.2) ;\fill[fill=black](1, 1) circle (2pt);
	\draw[line width=2pt] (5.1,5.3)--(5.1,6.8);
	\draw[line width=2pt] (5.1,7)--(5.1,8.5);
	\draw[line width=3pt](3.4, 6.9) ellipse (1.5 and 5);
	\draw[line width=3pt](6.8, 6.9) ellipse (1.5 and 5);
...
Die selbe Situation, aber aus einer anderen Perspektive auf das \glqq Connect 1\grqq \ Gerät zeigt die Abbildung  \ref{fig:xzFeld}. Eine die Elektromagnetischen Feldinien bilden Kreise radial um die Dipolstäbe. Die Abbildung  \ref{fig:xzFeld} zeigt das Feld bei y=0. Diese Felddarstellung beschreit das Feld in der xy Ebene.
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[!ht]%ESB Antenne
	\begin{center}
	\begin{tikzpicture}[scale = 0.8]
	\draw[line width=1.5pt](5, 5) rectangle (8, 6); %node at (9, 6) {Connect 1};
	\draw[line width=1.5pt, ->, >=latex](3, 3) -- (5,3 ) node at (5.2, 3) {x};
	\draw[line width=1.5pt, ->, >=latex](3, 3) -- (3, 5) node at (3.2, 5) {z};
	\draw[line width=1.5pt](3, 3) circle (0.2) ;\fill[fill=black](3, 3) circle (2pt);
	\draw [line width=2pt] (5.1,5.3)--(5.1,5.5);
	\draw[line width=3pt](5.1, 5.4) circle (1.5);
	\end{tikzpicture}
	\end{center}
...
Es wurden wie beim der Dipol Antenne in Kapitel \ref{sim_lamda2_Dipol}\ drei Simulationen durchgeführt. Diese sollen das Verhalten von Loop Aantennen mit Kunststoff im Nahfeld schematisch zeigen.\\ 
Die Resonanzfrequenz einer $\lambda/2$ langen Leiterschleife ist in der Abbildung  \ref{fig:S11_Loop_Lambda2_freiraum_1} gezeigt. Sie liegt bei dieser Simulation bei $5.84 GHz$.  Bei $5.84 GHz$ zeigt die Antenne einen $S_{11}$ Wert von mehr als -25dB. Die -10dB Bandbreite beträgt 400 MHz.\\
Das Smith Diagramm in  Abbildung \ref{fig:Smith_Loop_Lambda2_freiraum_2} zeigt die Fehlanpassung auf. Der Realteil der Antenne liegt bei 2721 Ohm und der Imaginärteil bei 6225 Ohm bei einer Frequenz von 2.45 GHz. Deshalb ist der Frequenzpunkt von $2.45 GHz$ beim rechten Ausgangspunkt des Smith-Diagramm. Dieser Punkt steht für eine $\infty$ Impedanz. Dies führt zu einer total Reflexion. Der Reflexionsfaktor $\Gamma$ ist in diesem Fall 1. Die vom EMPIRE XPU berechnete Abstrahleffizienz ist dementsprächent $1\%$.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=7cm]{content/bilder/Evaluation/Loop/L2/ohneABS/EM_Far_Field_Loop_Lambda2_ohneABS.JPG}%
	\caption{xxxx}
	\label{xxx}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[h]
	\centering
	\includegraphics[width=7cm]{content/bilder/Evaluation/Loop/L2/1ABS/EM_Far_Field_Loop_Lambda2_mitABS.JPG}%
	\caption{xxxx}
	\label{xxx}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
Die Loop Antenne wird nun zwischen zwei gleich grossen ABS Kunststoffstücken eingeklemmt. Die Antennenanordnung bleibt dieselbe. \\
Die erwartung ist, dass auch bei dieser Simulation ein Detunig statt findet.\\ Die Abbildung \ref{fig:S11_Lambda2_Loop_2ABS_5} zeigt, dass die Resonanzfrequenz weiter sinkt. Sie liegt neu bei 4.3GHz. Die -10dB Bandbreite beträgt 270 MHz. Der $S_{11}$ Wert zeigt bei 4.3GHz -13.2dB.\\
...
\begin{figure}[h]
	\centering
	\includegraphics[width=7cm]{content/bilder/Evaluation/Loop/L2/2ABS/EM_Far_Field_Loop_Lambda2_mit2ABS.JPG}%
	\caption{xxxx}
	\label{xxx}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
Die Abbildung \ref{fig:S11_Loop_freiraum_1} zeigt das die Rückfluss Dämpfung der Antennenstrucktur. Das Bild zwigt eine sehr schmalbandigen $S_{11}$ Verlauf. Die -10 dB Bandbreite beträgt 60MHz. In Abbildung \ref{fig:S11_Loop_freiraum_1} zeigt eine Antennenimpedanz von (37+j375)$\Omega$. Dabei resultiert eine Abstrahleffizien von ca. $\eta=2.5\%$
\begin{figure}[!ht]
\begin{center}
\minipage{0.4\textwidth}
...
Die beinen Abbildungen \ref{fig:S11_Loop_1ABS_3} und \ref{fig:Smith_Loop_1ABS_4} zeigen den dreifach Loop mit einem Angrenzenden Kunsstoff ABS Stück. Es zeigt sich, dass die -10dB Bandbreite enspricht 60MHz. Der maximale $S_{11}$ Wert zeigt sich bei 2.5 GHz mit einer Rückflussdämpfung von -14dB. Die simulierte Antennenimpedanz bei 2.45 GHz entspricht (19.7-j49)Ohm. Dies führt zu einer Abstrahleffizienz von 29$\%$.
...
Ein Loop mit drei Schleifen der zwischen 2 ABS Kunsstoffstücken positioniert ist, wird in der folgen Simulation gezegit.\\
Die maximale $S_{11}$ Dämpfung liebtt bei 2.45 GHz. Die -10dB Bandbreite beträgt 60MHz. Die Effizienz liegt bei einer Frequenz von 2.45Ghz bei $45\%$. Die Abbildung \ref{fig:Smith_Loop_2ABS_6} zeigt das Smith Diagramm. Bei der Entwurfsfrequenz von 2.45GHz resulitert eine Antennenimpedanz $Z_{ant}$ von (31.6+5.5)Ohm.
\begin{figure}[!ht]
\begin{center} 
\minipage{0.4\textwidth}
