...
\end{figure}
Die Oberfläche der elektromagnetischen Welle nimmt mit steigendem Abstand $r$ von der Antenne zu. Gleichzeitig nimmt die Beugung der Kugelwelle immer mehr ab. Ist der Abstand $r$ genügend gross, so kann die Welle lokal als ebene Wellenfront angenommen werden, was als Fernfeldkriterium bezeichnet wird. Bei Distanzen von mehr als $2\lambda$ kann das Fernfeld angenommen werden. Bei einer Sendefrequenz von 2.45 GHz einer Bluetooth Antenne tritt das Fernfeld wie in (\ref{eq:Fernfeld}) gezeigt nach 24 cm auf.
...
\label{eq:eta_rad}
\end{equation}
Die totale Abstrahleffizenz $\eta_{tot}$ wird gebildet aus:
\begin{equation}
...
\ref{fig:sim_Dipol_2ABS_3D} zeigen alle drei die 3DRichtdiagramme einer Dipolantenne bei einer Frequenz von 2.45 GHz. Sie zeigen ein qualitatives Abstrahlverhalten der Antennen. Die Richtwirkung aller drei Antennen ist etwa gleich sie seigen alle einen Torus um die Antenne. Das elektromagnetische Feld ist wie aus der Theorie in Kapitel xx vom Winkel $\theta$ abhängig.
