...
Die maximale Wirkleistung wird in einem Gleichstromkreis bei $R_Q = R_L$ abgegeben. Es besteht Leistungsanpassung das heisst die in dem $R_L$ umgesetzte Leistung ist maximal. Der Wirkungsgrad $\eta$ entsprich $50\%$, da die Selbe Leistung wie in der Last im Innenwiderstand der Quelle $R_Q$ umgesetzt wird. Die Formel \ref{eq:PmaxLeistungsanpassung} zeigt die maximale Leitung in der Last. Die Abbildung \ref{fig:LeistungsanpassungU0_RQ_RL} zeigt eine Schaltung bei der die Bedingung für Leistungsanpassung $R_Q = R_L$ gilt.
...
Gilt es eine hochohmige Last RL an eine Quelle mit einem RQ der kleiner ist als RL anzupassen, so können die Schaltung von Abbildung \ref{fig:LeistungsanpassungU0_RQkleiner_als_RL}und die mathematischen Beziehungen aud den Gleichungen \ref{eq:R1R2wennRLgrösserRQ}  verwendet werden.
