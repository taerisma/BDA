...
Ist ein Hertzscher Dipol unendlich dünn und in einem xyz Koordinatensystem im Ursprung und in die z Richtung ausgerichtet so gilt:
...
%	\label{Z0_Grafik}
%\end{figure}
Die Ausrichtung der E Feldlinien wechselt bei jeder Schwingung ihre Richtung. Im Nahfeld dominiert das E Feld. Mit wachsendem Abstand sind das E Feld und das H Feld senkrecht aufeinander und in Phase. Dabei können das E und H Feld als ebene Welle betrachtet werden. Die allgemeine Formel für die Feldverteilung im Kugelkoordinaten System lautet \cite{elliott1981antenna}:
...
\end{equation}
In weiter Ferne von dem Elementaren Strahler sind die Feldanteile $E_r$ soweit abgeklungen, dass sie als Null angenommen werden. Es bleiben ein E Feld Vektor und ein H Feld Vektor übrig. Vereinfacht könne die beiden E und H Feldvektoren als phasengleich und senkrecht zueinander angenommen werden. Die werden sich soweit im Raum ausbreiten, bis ihre gesamte Energie absorbiert ist. Das bedeutet die Amplitude des E und H Feldvektor wird Null.\\

Für alle Antennen die eine lange und dünne Geometrie besitzen, wie zum Beispiel die Monopolantennen, die Dipolantennen oder die Faltdipole bildet der Hertzsche Dipol die Grundlage der Feldausbreitung. Die Ausdehnung des Elementaren Strahlers ist auf ein Länge der Strahlenden Struktur von etwa $100\mu m$ beschränkt. Das bedeutet der Gültigkeitsbereich der in dieser Arbeit gezeigten Formeln und Beschreibungen führen bis 3 THz.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Die Elektromagnetischen Felder eines Hertzschen Dipols werden oft in Kugelkoordinaten beschrieben. Jeder Punkt kann mit einem Vektor $\vec{r}$ mit dem Start Punkt im Ursprung beschrieben werden. Die Spitze des Vektor $\vec{r}$ kann in die xy Eben projiziert werden. Der Winkel $\varphi$ gesprochen phi kann Werte $\pm \pi$ haben und dekt somit $360^\circ$ ab. Der Winkel $\theta$ gesprochen theta gibt die Neigung von der positiven z Achse an.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{figure}[h]
%\begin{center}
%\begin{tikzpicture}
