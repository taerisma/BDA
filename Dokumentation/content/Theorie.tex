...
Das zeitabhänige magnetische Moment \textit{m(t)} ergiebtsich durch die Multiplikation der durch die Schleife aufgespannten Fläche $a^{2}\pi=S$ und dem konstanten Schleifenstrom $I$ der auf der ganzen Schleife denselben Wert $I$ hat. Dank der Annhanme des konstanten Storm kann wie in Gleichung \ref{eq:magnetischesMoment} das magneische Moment berechtet werden. 
...
Der zentral gespeiste Dipol besteht meist aus runden Leiterstäben mit dem Durchmesser d, die so aneinander liegen, dass in der Mitte der beiden Stäbe eine kleine Lücke entsteht. Die gesamte Länge der beiden Stäbe entspricht $2l>>d$. Wird eine Spannung  in der Lücke zwischen den beiden Stäben angelegt, kommt es zu einer Stromverteilung über die gesammte Länge der  Stäbe. Oft wird die Spannung mit einer Zweidrahtleitung,  diese wird auch Transmission Line genannt, zwischen den Leiterstäben angebracht. Die anschliessende Stromverteilung der beiden runden Leiterstäbe liefert den Ursprung der Wellenausbreitung. In erster Näherung kann die sich von der Speisestelle ausbreitende Welle als richtungnsunabhänige Kugelwelle betrachtet werden \cite{elliott1981antenna}.
%E^j(wt-kr)/4pimu^-1 r
...
%Elliot Seite 63
Durch Auflösen der Doppelintegrale über $\phi$ von 0 bis $\pi$  und $\theta$ von 0 bis $\pi$ erhält man eine nummerische Lösung der Integrale über die gesammte Kugelfäche:
\begin{equation}
...
Wenn ein Dipol sehr kurz wird, zum Beispiel $2l=\lambda/8$ dann wird $R_{rad} = 3 Ohm$. Dieser Wert ist merklich kleiner als die 73 Ohm die  einens $\lambda/2$ Dipol. Der Effekt auf den reaktiven Anteil der Eingangsimpedanz ist noch dramatischer. Für einen endlich dünnen Dipol mit der Dicke d, ist die Reaktanz der Eingangsimpedanz eines $2l=\lambda/2$ Dipols positiv. Die Reaktanz ist wenig unter Null wenn die Dipollänge  $2l=\lambda/2$ enspricht. Wird der Dipol weiter gekürzt, so sinkt die Reaktanz sehr schnell  ins Negative. Im Fall, dass $2l=\lambda/8$ ist,  sind Werte für X grösser als 1000 Ohm kapazitive keine Seltenheit. 
...
Nimmt man zudem an, dass $ka$ klein ist, so kann der folgende Therm vereinfacht werden sin(ka $sin(\theta)$cos($\psi$))=ka sin($\theta$)cos($\psi$) so findet man 
\begin{equation}
...
\begin{itemize}
\item Als Sende- und Empfangchip wird beim Empfänger und Sender der CC2541 von TI eingesetzt
...
\label{fig:LinkModel}
\end{figure}
Die Abbildung \ref{fig:LinkModel} zeigt ein einfaches Verbinungsmodel einer Richtstrahlverbindung. Das in der Tabelle \ref{tab:Linkbudget} aufgezeigt Link Buget beschreibt eine Übretragung wie in Gleichung \ref{eq:LinkBudgetGelichung} gezeigt wrid. Es werden einige Vereinfachung getroffen. Der Gewinn der Empfangsantenne $G_{r}$  ist als Faktor 1, das enstprich 0dBi angenommen. Es werden keine Polarisationsverluste berücksichtigt. Für die berechnung der Freiraumdämpfung $L_{fs}$ wird als Übertragungsmedium Vakuum angenommen.
\begin{eqnarray}
...
\begin{table}[h]
  ...
  Erwartete Leistung am Empfänger & $P_{Rx}$  & -66dBm  \\ \midrule
  Systemreserve        			 & $L_{div}$&-6dBm \\ 
  Minimal notwendiges Empfängersignal &$P_{Rx}^{'}$ & -72dB \\ \bottomrule
  \end{tabular}
  \caption{Linkbudget 10m Bluethooth Übertragung}
  \label{tab:Linkbudget}
\end{table}

\subsection{Speisung}
Unter der Speisung der Antenne wird die Leistungszufühung verstanden. Damit eine Antenne strahlt, muss diese mit einer Spannungswelle angeregt werden. Die Strom und Spannungsverteilung der Antenne ist für das Abstrahlverhalten verantwortlich. 
...
$Z_{ein}=Z_{aus}*$
Das bedeutet, dass der Realanteil von $Z_{ein}$ und $Z_{aus}$ gleich ist, aber der Imaginäranteil von $Z_{aus}$ muss den konjugiertkomplexen Wert des $Z_{ein}$ aufweisen. Mit anderen Worten, $Z_{aus}$ hat beim Imaginärenanteil ein umgekehrtes Vorzeichen als der $Z_{ein}$. Leistungsanpassung kommt bei Leistungsendstufen oder allgemein dort zur Anwendung, wo es besonders wichtig ist, dass möglichst viel der erzeugten Leistung von der Last aufgenommen wird. Man bedenke, das ist im besten Falle nie mehr als 50\%.
...
\begin{figure}
	...
	\end{tikzpicture}
	\end{center}
\caption{Anpassung von Quelle und Antenne}
