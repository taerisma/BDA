...
\end{figure}
Aus der Elektrotechnik ist bekannt, dass sich beim Einschalten einer Signalquelle auf der Leitung ein Strom $i(t)$ einstellt. Die Folge davon ist ein magnetisches Feld radial um die Leitung. Der magnetische Fluss ergibt sich mit der Abschnittsinduktivität zu $\Phi = i  \Delta L$. Das Einschalten der Quelle führt weiter zu einer Spannung zwischen den Leitern, was wiederum zu einem elektrischen Feld und einer Oberflächenladung $Q = u  \Delta C$ auf den Leitern führt. Diese beiden Sachverhalte sind in den Ablildungen \ref{fig:Mag_Fluss_Leitung}	 und \ref{fig:OberfaechenLadung} dargestellt, wobei es sich um eine statische Feldbetrachtung der transversalen Ebene  einer Zweidrahtleitung handelt \cite{Tekom}.
\begin{figure}[!h]
\begin{minipage}{7cm}
	\centering
	\begin{tikzpicture}
...
\begin{figure}[!ht]
	\begin{center}
	\begin{tikzpicture}
		\draw[line width=1pt] (0, 5) -- (5, 5) node at(2.25,5.75){$Z_0$};
		\draw[line width=1pt] (0, 6.5) -- (5, 6.5);
		\draw[line width=1pt](5, 5.75) ellipse (0.2 and 0.75);
		\draw[line width=1pt] (5, 5.75) -- (6, 5.75);
		\draw[line width=1pt] (5, 5) -- (6, 5);
		
		\draw[line width=1pt] (6, 5.75) -- (6, 7.75);
		\draw[line width=1pt] (6, 5) -- (6, 3);
		
		\draw[line width=1pt] (6, 7.75) -- (9, 7.75);
		\draw[line width=1pt] (6, 3) -- (9, 3);
		
		\draw[line width=1pt] (9, 7.75) -- (9, 6.25);
		\draw[line width=1pt] (9, 3) -- (9, 5.25);
		
		\draw[line width=1pt](8.75, 5.25) rectangle (9.25, 6.25)node at(8.25,5.75){$Z_L$};
		
		\draw[line width=1pt, ->, >=latex](3, 4.5) -- (1.5, 4.5) node at(2.25,4){$U_R,I_R$};
		\draw[line width=1pt, ->, >=latex](1.5, 7) -- (3, 7)node at(2.25,7.5){$U_V,I_V$};
		
		\draw[line width=1pt, ->, >=latex](7.2, 8.25) -- (8.2, 8.25)node at(7.7,8.75){$I_L$};
		\draw[line width=1pt, ->, >=latex](9.75, 6.5) -- (9.75, 5)node at(10.25,5.75){$U_L$};
	\end{tikzpicture}
	\end{center}
...
Die Spannung $U_L$ über der Last ergibt sich aus der Summe der vorwärts und der rückwärts laufenden Spannugswellen, wie  in (\ref{eq:AnpassungULast}) gezeigt. Der in der Last fliessende Strom $I_L$ wird aus der vorwärts und der rückwärts laufenden Stromwelle gemäss (\ref{eq:AnpassungILast}) berechnet \cite{Tekom}:
...
Für die Anpassung einer Generatorimpedanz $R_Q$ an die grössere Last $R_L$ kann die Schaltung von Abbildung \ref{fig:Leistungsanpassung_RQ_grösser_als_RL} verwendet werden. Die Widerstände R1 und R2 gemäss der Gleichungen (\ref{eq:R1R2wennRQgrösserRL_R1}) und (\ref{eq:R1R2wennRQgrösserRL_R2}) \cite{Tekom}:
%\begin{equation}
	\begin{align}
...
Gilt es eine hochohmige Last $R_L$ an eine Quelle mit einem $R_Q$ kleiner als $R_L$ anzupassen, so kann die Schaltung in Abbildung \ref{fig:LeistungsanpassungU0_RQkleiner_als_RL} sowie für die Berechnung der Widerstände R1 und R2 die Beziehungen aus den Gleichungen (\ref{eq:R1R2wennRLgrösserRQ_R1}) und (\ref{eq:R1R2wennRLgrösserRQ_R2}) verwendet werden \cite{Tekom}:
...
\begin{equation}
Z_{Links}= R_{Links}+jX_{Links}=\dfrac{R_QX_P^2+jR_Q^2X_P}{R_Q^2+X_P^2}
\end{equation}
$X_P$ kann nun so gewählt werden, dass der Realteil von $Z_{Links}$ dem Lastwiderstand $R_L$ entspricht: \\
\begin{equation}
R_{Links}= R_L
\end{equation}
Als nächstes gilt es, die imaginäre Grösse $X_{Links}$ zu kompensieren, indem die Seriereaktanz $X_{S}$ auf der rechten Seite der Trennlinie entsprechend gewählt wird: 
\begin{equation}
X_{Links}= -X_S
\end{equation}
Je nachdem, ob $X_P$ induktiv oder kapazitiv ist, wird $X_S$ gewählt. Nun können die Werte der Induktivität $L$ und der Kapazität $C$ für die gewünschte Frequenz berechnet werden, indem die folgenden Formeln (\ref{eq:XL}) und (\ref{eq:XC}) entsprechend umgestellt werden.
\begin{align}
...
\begin{figure}[!ht]
	...
	\draw[line width=1.5pt, style=dashed](7, 1) -- (7, 5.5) node at (6, 1) {$Z_{Links}$};%gestrichelt
	...
	\end{tikzpicture}
	\end{center}
\caption{Verlustfreies L-Netzwerk}
...
\begin{figure}[t!]
	...
	\end{tikzpicture}
	\end{center}
\caption{Ersatzschaltbild einer Quelle mit Anpassungsnetzwerk und Antenne}
